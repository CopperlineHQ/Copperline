...
the sixteen frames before the target, which refills the Layer III bit
reservoir (at most 511 bytes back) and the one-frame-deep overlap-add and
synthesis state, so a sector decodes to the same bytes whichever way the
cursor reached it; that is what keeps a run resumed from a save state
byte-identical to an uninterrupted one, and a unit test holds it to that.
