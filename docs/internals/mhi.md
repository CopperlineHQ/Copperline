...
- **`CAPS`** (`0x02`, RO) -- a bitmask of the MPEG formats and bitrate
  ...
  M1-M2 sets bits 0-5 (MPEG-1/2/2.5, Layer III, CBR, VBR-no-seek); **M4**
  adds bit 6 (`VERSION` 2 -- see [Versioning](#versioning)). Layer I/II
  are not implemented, so bits for them do not exist in this version; a
  future version that adds them would need a `CAPS` bit for each, which
  is exactly the kind of change that bumps `VERSION`. This register is
  what the guest library's `MHIQuery` handler consults for
