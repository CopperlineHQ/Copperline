...
# midi_out = "FluidSynth"  # midi mode: host destination, "mt32", or "gm"
...
- `midi` -- serial in/out is bridged to host MIDI endpoints. Needs a build
  ...
  [the MT-32 chapter](mt32.md). `midi_out` may also be `"gm"` --
...
Coppersynth's own keys, in force when `[serial] midi_out = "gm"`. The
...
holding one; the `COPPERLINE_GM_SOUNDFONT` environment variable and an
