...
while developing -- is a headless run: no window opens, the core runs
unthrottled, and the result is reproducible.

Capture runs (`--screenshot-after`, `--dump-frames`) never connect to the
host's display server at all, so they work in environments without one:
SSH sessions, CI runners, and sandboxes that block window-server access.
