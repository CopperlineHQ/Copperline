...
;     to the end of its own frame: the frame's last raster line forces a
;     reset, so the display runs into the bottom border but the next
;     frame's top starts closed until DIWSTRT matches again. vAmiga
;     forces its vertical flipflop off at line 312 PAL / 262 NTSC the
;     same way; Copperline originally carried the flop across the blank
;     and lit the whole top of the frame, which this probe caught.
;
...
;            never reached -> open to the frame's last raster line, where
;            the forced reset ends it; the top of the frame stays border)
;
...
.fp:    move.w #$ff00,(a0)+       ; 8-on/8-off stripes
        ...
        move.l #$00e20000,(a1)+
        move.l #$009084c1,(a1)+   ; DIWSTOP: close the carried window at 132
